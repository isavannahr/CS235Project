...
def k_means_pp(count_xy ,k, plot_name):
    ...
    return centroids, nearest_centroid, all_distances
...
def plot(data,final_centroids, nearest_centroid,outliers,outlier_labels, plot_name):
    ...
    #print(outliers)
    plt.scatter(outliers[:,0],outliers[:,1], marker='*', c = 'gold', s = 150,label = 'outliers')
    plt.legend()

    for i in range(len(outlier_labels)):
        plt.annotate(
            outlier_labels[i],
            xy=(outliers[:,0][i], outliers[:,1][i]), xytext=(-20, 0), size = 8, color = 'k',
...
def plott(data, final_centroids, nearest_centroid):
    ...
    plt.xlabel("Number of Gun Laws")
    plt.ylabel("Number of Mass Shootings")
    plt.title("Removed Outlier Clusters")
    plt.scatter(final_centroids[0:, 0][:, 0], final_centroids[0:, 0][:, 1], c='r', s=100)
...
if __name__ == '__main__':
    ...
    count_xy, labels = merge_data(gunlawCounts_perStateYearDict, massshootingCounts_perStateYearDict)

    #2 clusters
    final_centroids, nearest_centroid, distances = k_means_pp(count_xy, 2,"Plots/initialcentroids_two_2016.png")
